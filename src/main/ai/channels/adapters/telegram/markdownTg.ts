...
export function normalizeModelHtmlBreaks(md) {
  ...
  t = t.replace(/\*\*[^*\n]+\*\*/g, (m) => (/<br\s*\/?>/i.test(m) ? hold(m.replace(/<br\s*\/?>/gi, ' ')) : m))
  t = t.replace(/(?<!\*)\*[^*\n]+\*(?!\*)/g, (m) => (/<br\s*\/?>/i.test(m) ? hold(m.replace(/<br\s*\/?>/gi, ' ')) : m))
  t = t.replace(/\[[^\]\n]+\]\([^)\n]+\)/g, (m) => (/<br\s*\/?>/i.test(m) ? hold(m.replace(/<br\s*\/?>/gi, ' ')) : m))
  t = t.replace(/<br\s*\/?>/gi, '\n')
...
const CELL_BR_PLACEHOLDER = 'TGCELLBRPLACEHOLDER'
...
function escapeInlineCellHtml(text) {
  const raw = String(text ?? '')
  if (!raw) return '—'
  if (CELL_BR_RE.test(raw)) {
    const withPh = raw.replace(CELL_BR_RE, CELL_BR_PLACEHOLDER)
    const rendered = escapeInline(withPh)
    return rendered
      .split(CELL_BR_PLACEHOLDER)
      .map((p) => p.trim())
      .filter(Boolean)
      .join('<br>')
  }
...
function listField(prefix, headerHtml, cellRaw) {
  ...
  return v.startsWith('\n') ? `${prefix} <b>${headerHtml}</b>:${v}` : `${prefix} <b>${headerHtml}</b>: ${v}`
}
  ...
  return rows
    .map((row) => {
      ...
      if (headers.length === 2) {
        ...
        let title
        if (CELL_BR_RE.test(rawTitle)) {
          const withPh = rawTitle.replace(CELL_BR_RE, CELL_BR_PLACEHOLDER)
          const rendered = escapeInline(withPh)
          title =
            rendered
              .split(CELL_BR_PLACEHOLDER)
...
export function normalizeLooseMarkdownTables(md) {
  ...
  while (i < lines.length) {
    ...
    if (
      ...
      const block = [q0 + ensureOuterPipes(r0), q1 + ensureOuterPipes(r1)]
      let j = i + 2
